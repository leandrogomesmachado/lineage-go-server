...
func montarItemListPacket(itens []gsdb.CharacterItem) []byte {
	return montarItemListPacketComJanela(itens, false)
}
...
func montarSkillListPacket(skills []gsdb.CharacterSkill) []byte {
	...
	escritor.escreverD(uint32(len(skills)))
...
func montarItemListPacketComJanela(itens []gsdb.CharacterItem, mostrarJanela bool) []byte {
	escritor := novoEscritorPacket()
...
func montarShortCutInitPacket(atalhos []gsdb.CharacterShortcut) []byte {
	escritor := novoEscritorPacket()
